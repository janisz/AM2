...
\begin{array}{l l l}
\text{Dolna suma całkowa:}& $s_n = s_n(\pi_n) = \sum\limits_{i=1}^{k_n}m_i^{(n)}(x_i^{(n)}-x_{i-1}^{(n)}})$ & $m\limits_i^{(n)} = \inf (f(x)) \quad x\in [x\limits_{i-1}^{(n)},x\limits_{i}^{(n)}]$\\
\text{Górna suma całkowa:}& $S_n = S_n(\pi_n) = \sum\limits_{i=1}^{k_n}M_i^{(n)}(x_i^{(n)}-x_{i-1}^{(n)}})$ & $M\limits_i^{(n)} = \sup (f(x)) \quad x\in [x\limits_{i-1}^{(n)},x\limits_{i}^{(n)}]$\\
\text{Suma całkowa Riemanna:}& $\sigma_n = \sigma_n(\pi_n) = \sum\limits_{i=1}^{k_n}f(\xi_i^{(n)})(x_i^{(n)}-x_{i-1}^{(n)}})$
\end{array}
\end{df}
...
$A \text{ -- przedział } \subset \mathbb{R}$ są różniczkowalne w każdym punkcie przedziału A. Ciąg $(f_n')$ jest z zbieżny jednostajnie na A, czyli $\exists x_0 \in A  \quad (f_n'(x_0))$ jest zbieżny $\Rightarrow$
\begin{itemize}
